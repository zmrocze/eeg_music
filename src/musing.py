...
songs_info_enhanced = {
    1: {
        "name": "Trip to the lonely planet",
        "artist": "Mark Alow",
        "genre": "Deep House",
        "duration": 125,
        "tempo": 121.95,
        "characteristics": "Electronic, Western",
    },
    2: {
        "name": "Sail",
        "artist": "Awolnation",
        ...
        "duration": 114,
        "tempo": 119,
        "characteristics": "Rock, English lyrics",
    },
    3: {
        "name": "Concept 15",
        "artist": "Kodomo",
        ...
        "duration": 132,
        "tempo": 161,
        "characteristics": "Experimental electronic",
    },
    4: {
        "name": "Aurore",
        "artist": "Claire David",
        ...
        "duration": 111,
        "tempo": None,
        "characteristics": "Ambient, meditative",
    },
    5: {
        "name": "Proof",
        "artist": "Idiotape",
        ...
        "duration": 124,
        "tempo": 123,
        "characteristics": "Danceable electronic",
    },
    6: {
        "name": "Glider",
        "artist": "Tycho",
        ...
        "duration": 100,
        "tempo": 126,
        "characteristics": "Atmospheric, instrumental",
    },
    7: {
        "name": "Raag Bihag",
        ...
        "duration": 116,
        "tempo": 70,
        "characteristics": "Indian classical, traditional",
    },
    8: {
        "name": "Albela sajan",
        ...
        "duration": 121,
        "tempo": 194,
        "characteristics": "Indian, Hindi lyrics",
    },
    9: {
        ...
        "duration": 126,
        "tempo": 117,
        "characteristics": "Indian folk, Gujarati lyrics",
    },
    10: {
        "name": "Fallin",
        "artist": "Dr. SaxLove",
        ...
        "duration": 129,
        "tempo": 197,
        "characteristics": "Jazz, instrumental",
    },
    11: {
        "name": "Master of Running",
        "artist": "Rickeyabo",
        "genre": "Goth Rock",
        "duration": 113,
        "tempo": 120,
        "characteristics": "Dark, alternative rock",
    },
    12: {
        "name": "JB",
        "artist": "Nobody.one",
        ...
        "duration": 117,
        "tempo": 146,
        "characteristics": "Complex, instrumental",
    },
}
...
class MUSINGDataset:
    ...
    def load_complete_dataset(self, max_subjects=None, verbose=True):
        ...
        subjects_to_load = (
            self.subjects[:max_subjects] if max_subjects else self.subjects
        )

        print("🔄 Loading complete MUSIN-G dataset...")
        ...
        for i, subject in enumerate(subjects_to_load, 1):
            ...
            for session in self.sessions:
                ...
                try:
                    ...
                    ratings_mask = (self.behavioral_data["Subject"] == int(subject)) & (
                        self.behavioral_data["Song_ID"] == int(session)
                    )
                    ratings: pd.DataFrame = self.behavioral_data.loc[
                        ratings_mask
                    ].copy()
                    ...
                    if not ratings.empty:
                        enjoyment = float(ratings["Enjoyment"].iloc[0])
                        familiarity = float(ratings["Familiarity"].iloc[0])
                    else:
                        enjoyment = None
                        familiarity = None
                    ...
                    raw.info["description"] = (
                        f"Song {session}: {song_info['name']} - {song_info['artist']}"
                    )
                    ...
                    if verbose:
                        print(
                            f"  ✓ Song {session}: {song_info['name']} ({song_info['genre']}) | "
                    ...
                    if verbose:
                        ...
                        if "File does not exist" in error_msg:
                            ...
                            if len(lines) > 2 and "Did you mean" in lines[1]:
                                print(
                                    f"  ✗ Song {session}: File naming issue - trying run number without zero padding"
                                )
                            else:
    ...
    def get_dataset_statistics(self):
        ...
        print(
            f"• Total expected files: {len(self.subjects)} × {len(self.sessions)} = {len(self.subjects) * len(self.sessions)}"
        )
        ...
        for session_num, song_info in self.songs_info.items():
            genre_ratings = self.behavioral_data[
                self.behavioral_data["Song_ID"] == session_num
            ]
            if not genre_ratings.empty:
                mean_enjoy = genre_ratings["Enjoyment"].mean()
                mean_familiar = genre_ratings["Familiarity"].mean()
                print(
                    f"  {song_info['genre']:25} | Enjoy: {mean_enjoy:.1f}/5 | Familiar: {mean_familiar:.1f}/5"
                )
