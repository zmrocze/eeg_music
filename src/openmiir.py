...
class OpenMIIRLoader:
        ...
        for run in self.runs[:max_runs]:
            try:
                ...
                self.data[subject_id][run] = {
                    ...
                    "n_trials": (
                        len(processed_events["condition_events"])
                        if processed_events
                        else 0
                    ),
                }
                ...
                trial_count = (
                    len(processed_events["condition_events"]) if processed_events else 0
                )
                print(
                    f"  ✓ Run {run}: {raw.times[-1]:.1f}s, {len(events_df)} events, {trial_count} trials"
                )
    ...
    def _process_events(self, events_df):
        ...
        try:
            ...
            condition_events["emotion_description"] = condition_events[
                "trial_type"
            ].map(
    ...
    def load_all_subjects(
        self, max_subjects=None, max_runs_per_subject=None, verbose=True
    ):
        ...
        if verbose:
            print("🔄 Loading OpenMIIR dataset...")
            print(
                f"📦 Subjects to load: {len(subjects_to_load)} of {len(self.subjects)}"
            )
    ...
    def get_dataset_statistics(self):
        ...
        if not self.data:
            print(
                "No data loaded. Call load_all_subjects() or load_subject_data() first."
            )
            return
        ...
        for subject_id, subject_data in self.data.items():
            ...
            subject_trials = sum(
                run_data.get("n_trials", 0) for run_data in subject_data.values()
            )
            ...
            for run_data in subject_data.values():
                if (
                    run_data.get("processed_events")
                    and "condition_events" in run_data["processed_events"]
                ):
                    conditions = run_data["processed_events"]["condition_events"][
                        "trial_type"
                    ].tolist()
        ...
        for subject_id, subject_data in self.data.items():
            for run_id, run_data in subject_data.items():
                if (
                    ...
                    matching_trials = condition_events[
                        condition_events["trial_type"] == condition_code
                    ]
...
def main(root_path="/home/zmrocze/studia/uwr/magisterka/datasets/openmiir"):
    ...
    print("=" * 50)
    print(
        f"Available subjects: {loader.get_available_subjects()[:5]}... (showing first 5)"
    )
    print()

    # Load a few subjects for demonstration
    print("📦 Loading first 3 subjects (2 runs each)...")
    loader.load_all_subjects(max_subjects=3, max_runs_per_subject=2)
    ...
    print("# loader.load_all_subjects()")
    print("# ")
    print("# Load specific subject:")
    print("# subject_data = loader.load_subject_data('01')")
    print("# ")
    ...
    print("# ")
    print("# Access raw EEG data:")
    print("# raw = loader.data['01']['1']['raw']  # Subject 01, Run 1")
    print("# events = loader.data['01']['1']['processed_events']['condition_events']")
