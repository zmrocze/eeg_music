...
from bcmi import (
    ...
    BCMICalibrationLoader, BCMITrainingLoader, BCMITestingLoader,
    BCMITempoLoader, BCMIScoresLoader, BCMIFMRILoader
)
...
class TestBCMILoaders(unittest.TestCase):
                ...
                for run_key, run_data in session_data.items():
                    ...
                    if 'calibration_trials' in processed:
                        ...
                        if trials:
                            ...
                            print(f"  ✓ Calibration trials structure verified")
                    
                    print(f"  ✓ Loaded subject {subject}, run {run_key}: "
                ...
                for run_data in session_data.values():
                    ...
                    if 'training_pairs' in processed:
                        ...
                        if pairs:
                            ...
                            print(f"  ✓ Training pairs structure verified")
                    break
                break
    ...
    def test_bcmi_fmri_loader(self):
        ...
        expected_tasks = ['classicalMusic', 'genMusic01', 'genMusic02', 'genMusic03', 'washout']
        ...
        if loader.subjects:
            ...
            print(f"  ✓ Uses task names instead of run numbers")
...
class TestMUSINGDataset(unittest.TestCase):
    ...
    def test_musing_initialization(self):
        ...
        print(f"  ✓ Found {len(dataset.subjects)} subjects, {len(dataset.sessions)} sessions")
                ...
                for session_data in subject_data.values():
                    if 'raw' in session_data:
                        ...
                        if metadata:
                            ...
                            print(f"  ✓ Retrieved metadata from raw object")
                        break
                break
...
class TestNMEDTLoader(unittest.TestCase):
    ...
    def test_nmedt_load_cleaned_eeg(self):
        ...
        for song_num, data in eeg_data.items():
            ...
            print(f"  ✓ Song {song_num}: {data['song_info']['title']} - "
    ...
    def test_nmedt_load_raw_eeg(self):
        ...
        raw_data = loader.load_raw_eeg_data(participant_ids=[1, 2], recording_sessions=[1])
        ...
        if raw_data:
            for (participant_id, session), data in raw_data.items():
                ...
                shape = data['data'].shape
                self.assertEqual(shape[0], 129)  # Electrodes (including vertex reference)
            ...
            print("  ⚠ No raw data files found (this is normal if only cleaned data is available)")
...
class TestOpenMIIRLoader(unittest.TestCase):
    ...
    def test_openmiir_load_subject(self):
        ...
        if loader.subjects:
            ...
            for run_id, run_data in data.items():
                ...
                print(f"  ✓ Subject {subject}, Run {run_id}: "
            ...
            for run_data in data.values():
                ...
                if processed:
                    ...
                    if not condition_events.empty:
                        ...
                        print(f"  ✓ Emotion annotations added successfully")
    ...
    def test_openmiir_emotional_states(self):
        ...
        for code in range(1, 10):
            ...
            print(f"  ✓ State {code}: {state_info['description']} "
...
class TestDataLoaderIntegration(unittest.TestCase):
    ...
    def test_all_loaders_instantiate(self):
        ...
        if dataset_exists(MUSING_ROOT):
            try:
                loader = MUSINGDataset(str(MUSING_ROOT))
    ...
    def test_common_interface(self):
        ...
        if dataset_exists(BCMI_ROOT / "bcmi-calibration"):
            loaders_to_test.append(("BCMI", BCMICalibrationLoader(str(BCMI_ROOT / "bcmi-calibration"))))
        ...
        for name, loader in loaders_to_test:
            for method in common_methods:
                self.assertTrue(hasattr(loader, method), 
                              f"{name} loader missing method: {method}")
    ...
    def test_error_handling(self):
        ...
        try:
            loader = BCMICalibrationLoader(fake_path)
        ...
        try:
            loader = OpenMIIRLoader(fake_path)
            # Should not raise during initialization
